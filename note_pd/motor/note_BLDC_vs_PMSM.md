...
由於方波驅動切換過程不連續, 會產生轉矩波動, 而引發振動和噪音
> 低速下尤其如此, 電機不會均勻地旋轉
...
>> 與方波驅動相比, 正弦波驅動過程較為連續, 產生的轉矩波動也小(振動小噪音低), 其效率更高, 較適合高轉速應用
...
以永久磁鐵當轉子, 使用 `方波(梯形波)` 去驅動 Inverter (反電動勢亦為方波), 讓定子產生磁場, 藉此轉動 motor
...
使用永久磁鐵當轉子, 使用 `正弦波` 去驅動 Inverter (反電動勢亦為正弦波)
> 採用 FOC (Field-Oriented Control, 向量控制)技術, 一個電週期一般最少會有 18 個向量 (當然越多越好), 需要高性能的 MCU 或 DSP 才能實現

+ FOC 種類
    > + SPWM 演算法
    > + SVPWM 演算法

# 啟動馬達流程 (Starting method of motor)


## 方波啟動流程

![Start_method_square](Start_method_square.jpg)

在 sensorless 方波驅動的情況下, 啟動時不會產生感應電動勢, 無法識別轉子位置, 因此需要先識別轉子位置
> + 首先強行使電流流向指定線圈(定子), 使轉子移動到指定位置
>> 強行移動到指定位置時, 可能會因停止位置而瞬間反轉
> + 使電流在低電壓和恆定週期的線圈(定子)中流動
> + 再逐漸增加電壓的同時旋轉轉子
> + 增加轉數, 轉子位置可以通過感應電壓檢測
> + 使電流流向檢測到的轉子位置所對應的線圈
> + 如果能檢測到穩定的位置, 則開始穩定運行以達到目標速度

電流流動有6種模式: U->V, U->W, V->W, V->U, W->U, W->V
> 每 60 度切換一次電流方向

![rotate_sequency_square](rotate_sequency_square.jpg)

## 正弦波啟動流程

![Start_method_sin](Start_method_sin.jpg)

無法使用正弦波驅動來啟動電機, 因為啟動時, 停止的轉子不跟隨頻率變化
因此在啟動電機時, 需要通過反饋轉子位置進行加速操作
> + 檢測轉子位置
> + 根據轉子位置, 對線圈(定子)緩慢施加正弦電壓
> + 在逐漸增加電壓的同時旋轉轉子
> + 根據位置檢測感測器的時間, 控制正弦波的週期
> + 轉子以一定的轉速旋轉時, 開始穩定運行, 以達到目標轉速


根據旋轉角度, 將線圈(定子)電壓轉換為正弦波狀態, 並形成旋轉磁場, 從而使轉子旋轉
> 通過對與旋轉同步的線圈(定子)施加正弦電壓, 可以使轉子連續旋轉

![rotate_sequency_sin](rotate_sequency_sin.jpg)
