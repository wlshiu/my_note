...
help()
{
    echo -e "usage: $0 <start date> <end date> <out file>"
    echo -e "  e.g. $0 2023-06-09 2023-09-12 CHANGELOG.md"
    exit -1;
}
...
git log --decorate --oneline --since="${start_date}" --until="${end_date}" > ${log_file}
...
./t.py -i ${log_file} -o ${out_file} -s ${start_date} -e ${end_date}
