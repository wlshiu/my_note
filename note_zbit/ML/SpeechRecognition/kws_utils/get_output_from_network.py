...
'''
@ Summary: 獲取wav音頻數據
@ Update:  1.0.2 計算wav的mfcc數據


@ file:    get_output_from_network.py
@ version: 2.0.0 獲取cnn 網絡的中間變量並輸出


@ version: 2.0.1 代碼重構

@ version: 2.0.2 保存每一個層輸出的最大值和最小值

@ Date:    2020/05/27
           需要對批量數據推理時的每一層的輸出；
           CNN好像出了點玄學問題，轉到office_get_layers_output.py 繼續更新
...
def main(_):
    ...
    for i in range(len(res)):
        print(f'第{i+1}層輸出的最大值是：{res[i].max()},'
              f' 最小值是：{res[i].min()}' + '\n')

        # with open(FLAGS.every_network_output, 'a') as f:
        #     f.write(f'第{i+1}層輸出的最大值是：{res[i].max()},'
        #             f' 最小值是：{res[i].min()}' + '\n')
