# coding=utf-8
"""
@ Summary: 讀取data數據集，生成10%的test_list & 10%的val_list
@ Update:  將隨機的分割改為按照文件的hash值進行篩選，
           與訓練時候篩選的方式保持一致
...
@ file:    data_clips.py
@ version: 1.0.3
@ Update:  保留wanted words, 進行訓練


@ file:    data_clips.py
@ version: 1.0.4
@ Update:  將非標籤文件保存進zero文件夾中
    ...
    for file in wav_paths:
        ...
        if ret == "testing":
            with open(test_txt, "a+") as f:
                print("將{}寫入test 文件中....".format(os.path.basename(file)))
                f.write(file[11:]+"\n")
            ...
            # 1. create dir
            cur_file = Path(file)
            parent = cur_file.parent  # 返回當前路徑的父路徑
        ...
        if flag:
            if ret == "validation":
                with open(val_txt, "a+") as f:
                    print("將{}寫入val 文件中....".format(os.path.basename(file)))
                    f.write(file[11:]+"\n")
            else:
                with open(train_txt, "a+") as f:
                    print("將{}寫入train 文件中...".format(os.path.basename(file)))
                    f.write(file[11:]+"\n")
...
if __name__ == "__main__":
    ...
    #
    # print(f"共有{test_file_nums}個測試文件")
    print("數據分類完成....")
