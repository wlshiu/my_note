...
def run_quant_inference(wanted_words, sample_rate, clip_duration_ms,
  ...
  # 輸出tf的日誌,有五個級別,DEBUG，INFO，WARN，ERROR和FATAL,默認是WARN,此處設為INFO
  ...
  predicted_indices = tf.argmax(logits, 1)  # 返回的是logits中的最大值的索引號
  ...
  for i in range(len(max_output)):
      if i == 0:
          ...
          with open(every_network_output, 'a') as f:
              f.write(f'input 輸出的最大值是：{max_output[i]},'
                      f' 最小值是：{min_output[i]}' + '\n')
      else:
          tf.logging.info(f'第{i}層的最大值是{max_output[i]}, 最小值是{min_output[i]}')
          with open(every_network_output, 'a') as f:
              f.write(f'第{i}層輸出的最大值是：{max_output[i]},'
                      f' 最小值是：{min_output[i]}' + '\n')
